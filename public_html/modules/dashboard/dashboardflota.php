<?php 
	...
	require_once('configexamen.php');
	$crud=new CrudUsuario();
	$crudex=new CrudExamen();
	$aleatorio = uniqid();	
	$puestoaid = 28; 
	$vehiculo = $_COOKIE['usuarioid'];	
    if ($usuarioestado==0){
	echo "Usuario bloqueado ".$accesos.'-'.$aid; 
	echo '<br><a  href="logout">Salir</a>';
	}else{
...
FROM `exa_detalle_user` WHERE fecha=:fecha AND centro=:centro AND id_tema=:id_tema AND tipo_pregunta IN ('4','7')
		...
		<td>
		<form  onsubmit="return confirm('<?php echo $aid; ?>, Confirmaras los resultados del vehiculo <?php echo $rows['user_registro']; ?>');" method='POST' action='dashboardflota?exa=check_supervisor&amp;grupo=<?php echo $rows['grupo']; ?>&amp;vehiculo=<?php echo $rows['user_registro']; ?>'>		
	    <button class="btn btn-<?php echo $checks; ?> btn-sm" <?php echo $checksdisabled; ?> >Check</button>
	    </form>			
		</td>	 
        <td></td>	 		
        <td><button type="button" class="btn btn-primary btn-sm" onclick="location.href='dashboardflota?exa=ver_check_list&id_tema=<?php echo $id_tema; ?>&user_registro=<?php echo $rows['user_registro']; ?>&fecha=<?php echo $fecha; ?>';" >Ver</button></td>
        <td><button type="button" class="btn btn-primary btn-sm" onclick="location.href='backlogflota?hc=backlogadd&id_tema=<?php echo $id_tema; ?>&user_registro=<?php echo $rows['user_registro']; ?>&fecha=<?php echo $fecha; ?>&grupo=<?php echo $rows['grupo']; ?>';" >BackLog</button></td>
		</tr>	
		<?php 
		$n++;	
		}
...
?>	
   <!-- Modal Inicio-->
	...
	<form method="post" action="dashboardflota">
	<div class="mb-3">
...
?>	
   <!-- Modal Inicio-->
	<div class="modal fade" id="ModalflotaExportar" role="dialog" aria-labelledby="Modalflotatema" aria-hidden="true">
	<div class="modal-dialog" role="document">
	<!-- Modal content-->
	<div class="modal-content">
	<div class="modal-header">
	<h5 class="modal-title">Exportar .xls</h5>
    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
    <span aria-hidden="true">&times;</span>
    </button>
	</div>
	<div class="modal-body">
	<form  class="needs-validation"  method="post" name="exportar" action="excel" >
	<div class="input-group">
	<span class="input-group-text">Fecha :</span>
	<input  aria-label="First name" class="form-control" placeholder="Fecha inicio" value="<?php echo $fecha; ?>" type="date" name='fecha_ini'>
	<input  aria-label="Last name" class="form-control" placeholder="Fecha fin" value="<?php echo $fecha; ?>" type="date" name='fecha_fin'>
	<input id="consulta" name="consulta" type="hidden" value="checklistrep">
	</div>
			<select name="checklisttema" class="form-control" required>
			<option value="">Seleccionar Check List</option>
			<?php		
			$db=Db::conectar();
			$select=$db->prepare("SELECT c.id,c.descripcion FROM (SELECT * FROM exa_temas WHERE estado=1 AND para=:para) AS c WHERE c.multicentro=1 OR c.centro=:centro ORDER BY c.id,c.descripcion");
			$select->bindValue('centro',$idcentro);
			...
			$select->execute();
			while ($regis=$select->fetch()){
			if(strtolower($registro['id']) == strtolower($regis['id'])) { $selected = "selected"; }
	...
	<div class="modal-footer">
	<button type="submit" class="btn btn-secondary btn-lg btn-block"  <?php echo $disableform; ?> >Exportar</button>
	<button type="button"  class="btn btn-danger btn-lg btn-block" data-dismiss="modal">Cerrar</button>
	</div>
	</form>
	</div>
...
switch ($exa):
	...
	header('Location: dashboardflota');	
    }
	break;
    default:
	mondal_flota();
    ...
    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
     ...
     <button type="button" class="btn btn-light" data-toggle="modal" data-target="#ModalflotaExportar">Exportar</button>
    </div>
	</div>
	</div>
	</div>
