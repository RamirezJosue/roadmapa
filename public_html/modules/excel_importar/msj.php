<?php
	...
	$dbrs=Db::conectar();
    $selectrs=$dbrs->prepare("SELECT SUM(a.Contar) as Contactos,MAX(a.updated_at) as actulizado FROM (SELECT 1 as Contar,updated_at FROM t77_msj WHERE centro='$idcentro') AS a");
	$selectrs->execute();
...
function tabla_fbl5n($Contactos,$actulizado){
   ...
   $cabezastb='centro,codcli,msj'; 
   $arrayHTB = explode(",", $cabezastb);
?>  <p class="text-danger">Registros: <?php echo $Contactos; ?>  Actualizado: <?php echo $actulizado; ?> 
	<div class="table-responsive">
		...
		?>
         ...
         <?php
		  $db=Db::conectar();
          $select=$db->prepare("SELECT * FROM t77_msj WHERE centro=:centro");
...
global $db,$idcentro,$idcentrotb;
			$db=DB::conectar();
			$detele=$db->prepare("DELETE FROM t77_msj WHERE centro=:centro");
...
switch ($op):
    case "borrarbdrs":
	borrar_rs_tem();
	header('Location: excel_importar_msj');
	echo "xx";
	
    break;
    case "enviarrsdb":
	    $sqlverifi="SELECT Fecha FROM t77_msj GROUP BY Fecha";
	    ...
	    if($crud->contardbuser('Fecha','t77_rs','Fecha = "'.$fecharsverifi.'" AND centro="'.$idcentro.'"')>0){
		   ...
		   <?php
         header('Refresh: 3; URL=http://www.bk77.co/excel_importar_msj');		   
		} else {
	      $db=Db::conectar();
    ...
    default:	
?>
<p class="h3"> Mjs hoja ruta : <?php echo $idcentro; ?></p>
<a href="img/formato_mjs_hoja_ruta.xlsx" class="badge badge-info" target="_blank" >Formato mensaje hoja de ruta</a>
<form action="excel_importar_msj" method="post" enctype="multipart/form-data">
   ...
   <button class="btn btn-danger btn-sm" type="button" onclick="location.href='excel_importar_msj?op=borrarbdrs';" <?php echo $disablegrabar; ?> >Borrar datos cargados</button>
   </div>
</form>
<?php	
  ...
  if(isset($_FILES['archivoexcel'])){
	...
	for ($row = 2;$row<=$filas;$row++){
		$codcli   = htmlspecialchars($hoja -> getCell('A'.$row)->getCalculatedValue(),ENT_QUOTES,'UTF-8'); //ok
        $msj   = htmlspecialchars($hoja -> getCell('B'.$row)->getCalculatedValue(),ENT_QUOTES,'UTF-8');	
			 $insert=$db->prepare("
			 INSERT INTO t77_msj 
(`id`, `centro`, `codcli`, `msj`, `updated_at`)
			 VALUES
(null,'$idcentro','$codcli','$msj','$fecha_hora')
			 ");  
