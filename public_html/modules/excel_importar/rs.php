	...
	<div class="table-responsive">
		...
		<th>Entrega</th>
         ...
         <?php
		  ...
		  FROM `t77_rs_temp_centros` AS a WHERE centro='$idcentro'
...
switch ($op):
    ...
    case "enviarrsdb":
		...
		//     $insert->execute();
			$db=Db::conectar();  
		 	$insert=$db->prepare("INSERT INTO `t77_rs` 
			 	SELECT 
				null AS `id`,
				CONCAT(a.`Ruta`,replace(a.`Fecha`, '-', ''),a.`Codigo`,a.`Viaje`) AS `indx` , 
				0 AS `entregado`,0 AS `alerta`,0 AS `rechazo`,'' AS `mr`, 0 `cjasrechazadas`,
				a.`centro`,
				'' AS `comentario`,'' AS `dni`,
				'0000-00-00 00:00:00' AS `fechahoraalerta`,'0000-00-00 00:00:00' AS `registrofin`,
				'' AS `autoriza_rech`,'' AS `clasifica_envases`,
				0 AS `cjasclasificadas`, 0 AS `repaso`,
				'' AS `encuesta`,0 AS `st_kpi`,0 AS `reprogramado`,'' AS `Transporte`,
				a.`Ruta`, a.`Vehiculo`, a.`Fecha`, a.`Codigo`, a.`TipoCliente`, a.`ZNPVTA`, a.`ZNTVTA`, 
				a.`Cliente`, a.`Direccion`, a.`Abre`, a.`Cierra`, a.`Viaje`, a.`DistKm`, a.`Sec2`, a.`Sec1`, 
				a.`Entrega`, a.`Recojo`, a.`Llega`, a.`Sale`, a.`Ciudad`, a.`Longitud`, a.`Latitud`,
				0 AS `HL`,0 AS `Packs`,'' AS `SupervisionID`,'' AS `SalesManagement`,0 AS `DistanciaPlan`,
				'00:00:00' AS `Inicio`,'00:00:00' AS `Fin`,
				0 AS `CodEt`,'' AS `TelefonoCli`,'' AS `TipoPedido`,
				0 AS `ValorContado`,0 AS `VarlorCredito`,0 AS `DeudaEnv`,0 AS `DeudaProd`,
				'' AS `Tipo_Riesgo`,'' AS `Servicio_Flex`,'' AS `Tipo_NPS`,0 AS `RMD`,'' AS `TIPO`,'' AS `Rechazos`
				FROM `t77_rs_temp_centros` AS a 
		  ...
		  if($lastInsertId>0){
		   ...
		   <?php 
		    header('Refresh: 3; URL=modules/index');
		  } else { 
		   ?> 
    ...
    default:	
?>
	<p class="h3"><i class="fa fa-truck" aria-hidden="true"></i> Hoja de ruta rs CD : <?php echo $idcentro; ?></p>
	...
	<form onsubmit="return confirm('Por favor revise los datos rutas, contactos y fecha... de ser correctos enviar.');" name="enviar" action="rs?op=enviarrsdbxxx" method='GET'>
		<input name="op" type="hidden" value="enviarrsdb">
