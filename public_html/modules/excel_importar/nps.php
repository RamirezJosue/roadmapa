...
function tabla_fbl5n($Contactos,$actulizado){
   ...
   $cabezastb='centro,codcli,Tipo_reclamo,observacion'; 
...
Registros : <?php echo $Contactos; ?>  Actualizado: <?php echo $actulizado;?> 
	...
	<div class="table-responsive">
		...
		?>
         ...
         <?php
		  $db=Db::conectar();
          $select=$db->prepare("SELECT `centro`, `codcli`, `Tipo_reclamo`, `observacion` FROM t77_nps WHERE centro=:centro");
...
?>
   ...
   <?php } else { ?>
   <form onsubmit="return confirm('Esta seguro de borrar, tendras que cargar nuevamente...');" name="borrar" action="nps" method='GET'>
		<input name="op" type="hidden" value="borrarbdnps">
