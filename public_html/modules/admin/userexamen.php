...
$estado_tema=$cuestiontb[2];   
    ...
    if($grupo[1]==0) {		
	?>
	<button type="button" class="btn btn-primary btn-sm" onclick="location.href='userexamen?exa=enviarchecklist&grupo=<?php echo $grupo[0]; ?>&id=<?php echo $id_tema; ?>';" >Enviar resultado ?</button>
	<?php
    ...
    <div class="col-sm-12">
    <h4>
    <small class="text-muted"><?php echo ucfirst($desc_tema); ?><?php echo ' :: '.$tipocuestionario[$tipo_tema].' - '.$aid; ?></small>
    </h4>
	</div>
	</div>
	<form action="userexamen?exa=iniciar&id=<?php echo $id_tema; ?>" method="POST">
    <div class="row">
    <div class="col-6">
	<button type="button" class="btn btn-primary btn-sm btn-block" onclick="location.href='userexamen';" >Cancelar</button>
	</div>
	<div class="col-6">
	<button type="submit" class="btn btn-danger btn-sm btn-block" <?php echo isset($disablegrabar)? $disablegrabar : ''; ?> >Grabar</button>
	</div>
	</div> 
    <br> 	
	<?php
...
<table class="table table-sm">
  ...
  <?php		
	...
	<form  onsubmit="return confirm('Se iniciara con el conductor seleccionado, no se podra revertir');" method='POST' action='userexamen?exa=crear_examen_check&amp;idtema=<?php echo $registema['id']; ?>'>		
		<td>
...
switch ($exa):
	...
	header('Location: userexamen?exa=iniciar&id='.$id_tema.'');
	exit();
	} else {
	header('Location: userexamen?exa=iniciar&id='.$id_tema.'');
	exit();
    ...
    header('Location: userexamen?exa=iniciar&id='.$_GET['id'].'');
