...
?>	
  ...
  <?php	
	...
	<button  type="button" class="btn btn-danger btn-sm" onclick="location.href='checklistrpt?exa=result&amp;id=<?php echo $registema['id']; ?>';" >Resultados</button>
		...
		<td> 
	<?php if($registema['id']==446336597) {?>  	
    <button  type="button" class="btn btn-danger btn-sm" onclick="location.href='excel?excelnps=nps&amp;id_tema=<?php echo $registema['id']; ?>';" >xls</button>
	<?php }else{ ?>  
	<button  type="button" class="btn btn-danger btn-sm" onclick="location.href='excel?excel=cheklist&amp;id_tema=<?php echo $registema['id']; ?>';" >xls</button>
	<?php }?>  
		</td>		
		</tr>	
		<?php 
...
?>	
	...
	<input type="button" class="btn btn-danger btn-sm" value="Volver" onClick="history.go(-1);">
	<div class="table-responsive">
...
GROUP BY  `id_tema`, `fecha`, `user_registro` , `codigo_cliente` , `grupo` 	
    ...
    default:	
	$sqltema ="
	SELECT r.id_tema,r.tema,r.fecha_registro,(SELECT CONCAT(apellidos,' ',nombre) FROM usuarios WHERE dni=r.auditor) as auditor,r.area,r.grupo,r.resultado FROM (
SELECT `id_tema`, (SELECT descripcion FROM exa_temas WHERE id=`id_tema`) as tema, `fecha` as fecha_registro, user_registro as auditor , `codigo_cliente` as area , `grupo`,  'NA' as resultado  FROM `exa_detalle_checklist` 
WHERE id_tema=$id AND centro=:centro AND st = 1
GROUP BY  `id_tema`, `fecha`, `user_registro` , `codigo_cliente` , `grupo` ) AS r ORDER BY 	r.fecha_registro DESC
	";				 
endswitch; 
		$dbtema=Db::conectar();
        $selecttema=$dbtema->prepare($sqltema);
