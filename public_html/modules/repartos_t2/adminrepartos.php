...
<table class="table table-sm table-striped table-hover border">
  ...
  <tbody>
    ...
    <tr>
    <td colspan="5">
	<form  onsubmit="return confirm('Esta seguro de enviar la información');" name="conductor" method='POST' action='adminrepartos?l=seg&amp;fecha=<?php echo $fechars; ?>&amp;ruta=<?php echo $rpr['Ruta']; ?>'>
  ...
  <div class="form-group">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
	  <form class="needs-validation" novalidate action="adminrepartos?st=det&amp;ruta=<?php echo $ruta; ?>&amp;fecha=<?php echo $fecha; ?>&amp;viaje=<?php echo $viaje; ?>" method="get"> 
...
SELECT * FROM `t77_rs_ruta_sif` WHERE centro = :centro AND fecha_plan = :fecha_plan
		...
		//echo  'si'; 
		$LinkHora1='adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=RegistraHora&fecha='.$FechaRS.'&ruta='.$Ruta.'&viaje='.$Viaje.'&vehiculo='.$Vehiculo.'';
		} else {
		//echo  'no';
    $LinkHora1='#';
    ...
    case (($salida_cd==$hora) && ($llegada_cd==$hora) && ($ingreso_cd==$hora) && ($fin_conductor==$hora)):
        $LinkHora2='adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=RegistraHora&fecha='.$FechaRS.'&ruta='.$Ruta.'&viaje='.$Viaje.'&vehiculo='.$Vehiculo.'&tb=salida_cd&id='.$idhr.'';
        break;
    case (($salida_cd!=$hora) && ($llegada_cd==$hora) && ($ingreso_cd==$hora) && ($fin_conductor==$hora)):
        $LinkHora3='adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=RegistraHora&fecha='.$FechaRS.'&ruta='.$Ruta.'&viaje='.$Viaje.'&vehiculo='.$Vehiculo.'&tb=llegada_cd&id='.$idhr.'';	
        break;	
    case (($salida_cd!=$hora) && ($llegada_cd!=$hora) && ($ingreso_cd==$hora) && ($fin_conductor==$hora)):
       $LinkHora4='adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=RegistraHora&fecha='.$FechaRS.'&ruta='.$Ruta.'&viaje='.$Viaje.'&vehiculo='.$Vehiculo.'&tb=ingreso_cd&id='.$idhr.'';	
        break;
    case (($salida_cd!=$hora) && ($llegada_cd!=$hora) && ($ingreso_cd!=$hora) && ($fin_conductor==$hora)):
        $LinkHora5='adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=RegistraHora&fecha='.$FechaRS.'&ruta='.$Ruta.'&viaje='.$Viaje.'&vehiculo='.$Vehiculo.'&tb=fin_conductor&id='.$idhr.'';	
        break;
	}
		}
	?> 
		...
		<button class="btn btn-danger btn-sm" onclick="location.href='adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=DetalleRuta&amp;ruta=<?php echo $registro['Ruta']; ?>&amp;fecha=<?php echo $registro['FechaRS']; ?>&amp;viaje=<?php echo $registro['Viaje']; ?>';">Ver</button>
		</td>		
		</tr>
...
?>
	...
	<button type="button" class="btn btn-danger btn-sm" onclick="location.href='adminrepartos?&fecha=<?php echo $fecha; ?>&e=<?php echo $hash; ?>';">Cancelar</button>
	...
	<button type="button" class="btn btn-success btn-sm" onclick="location.href='adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=AlertarWS&amp;id=<?php echo $registro['id']; ?>';"><i class="fa fa-whatsapp"></i> <?php echo $valuebutton; ?></button>
  ...
  ?>
    ...
    <div class="col-sm-6 border">	
        ...
        <button type="button" class="btn btn-primary" onclick="location.href='adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=DetalleRuta&amp;ruta=<?php echo $registro['Ruta']; ?>&amp;fecha=<?php echo $registro['Fecha']; ?>&amp;viaje=<?php echo $registro['Viaje']; ?>';">Cancelar</button>		
		<?php } else { 
		?> 
...
ON r.Codigo = d.codigocliente 
	...
	switch ($FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl):
	 ...
	 header('Refresh: 3; URL=adminrepartos?fecha='.$_GET['fecha'].'&e='.$hash.'');
	 } else {
	...
	header('Location: adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=DetalleRuta&ruta='.$_GET['ruta'].'&fecha='.$_GET['fecha'].'&viaje='.$_GET['viaje'].'');
	}
	...
	header('Refresh: 3; URL=adminrepartos_new');
	}
	break;
	    case "AlertarWS":
	rutas__ws($_GET["id"]);
	...
	if(isset($_GET['confirmarrchz'],$_GET['autoriza_rech'],$aid)){
		...
		header('Refresh: 3; URL=adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=AlertarWS&id='.$_GET['idrechazo'].''); 				
		} else {
	...
	echo '<div class="alert alert-success" role="alert">Rechazo registrado</div>';	
		header('Refresh: 1; URL=adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=AlertarWS&id='.$idrechazo.''); 			
		}
	} else {
	...
	echo '<div class="alert alert-success" role="alert">Alerta registrada, ahora se compartira por WS</div>';	
		 header('Refresh: 1; URL=adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=AlertarWS&id='.$_GET['idrechazo'].''); 	
	} else {
	echo '<div class="alert alert-danger" role="alert">Algo esta mal</div>';	
		 header('Refresh: 1; URL=adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=AlertarWS&id='.$_GET['idrechazo'].''); 		
	}
	...
	echo '<div class="alert alert-success" role="alert">Pedido entregado</div>';	
		 header('Refresh: 1; URL=adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=AlertarWS&id='.$_GET['IdEntrega'].''); 	
	} else {
	echo '<div class="alert alert-danger" role="alert">Algo esta mal</div>';	
		 header('Refresh: 2; URL=adminrepartos?FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl=AlertarWS&id='.$_GET['IdEntrega'].''); 		
	}
	break;
