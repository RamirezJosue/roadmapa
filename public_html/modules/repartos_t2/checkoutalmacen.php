...
function listar_productos($Ruta,$Viaje){
	global $idcentro,$fecha_hora,$fechars,$filename;
	if (isset ($_POST['checkpdt'])){
	...
	<div class="table-responsive">
     ...
     class="display compact cell-border">
		...
		<?php 
		  ...
		  $sql ="
		  SELECT * FROM
		  ( SELECT *  FROM `t77_checkin_pt` WHERE FechaPlan=:FechaPlan AND centro=:centro AND Ruta=:Ruta AND Viaje=:Viaje ) AS a LEFT JOIN
...
?>
	...
	break;	
	default:
	listar_productos(strtoupper($aid),1);
	endswitch;
