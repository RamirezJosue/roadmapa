...
?>
		...
		function rutas__()
		{
			global $idcentro, $fechars, $aid, $enlace_actual, $archivo,$usuarionombre;
		?>
			<div class="row">
		  ...
		  ";
								...
								?>
									<tr role="row">
										...
										<td>
                                            <button class="btn btn-danger btn-sm" data-toggle="modal" data-target="#cargacamion<?php echo $idruta; ?>" ><i class="fa fa-battery-quarter" aria-hidden="true"></i></button>
								...
								<?php
                                    truck__load($idruta,$Ruta,$fechars,$Viaje);
								}
								Db::desconectar();
								?>
							</tbody>
  ...
  ");
		...
		function truck__load($idruta,$ruta,$fecha,$viaje){
            global $aid,$usuarionombre,$idcentro;
            ?>
                <div class="modal fade" id="cargacamion<?php echo $idruta; ?>" tabindex="-1" aria-labelledby="cargacamionL" aria-hidden="true">
                <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                    <h5 class="modal-title" id="cargacamionL"><?php echo $ruta.' | '.$fecha.' | '.$viaje ; ?></h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                    </div>
                    ...
                    <?php
                    $db=Db::conectar();
                    $sql ="SELECT * FROM `t77_checkin_cd` WHERE Centro='$idcentro' AND Ruta='$aid' AND Conductor='$usuarionombre' AND Viaje = '$viaje' AND FechaPlan = '$fecha' ORDER BY TpMt DESC";
                    $select=$db->prepare($sql);
                    $select->execute();
                    while ($registro=$select->fetch(PDO::FETCH_ASSOC)){
                    if($registro['TpMt']=='BLGA'){ $clasT = 'text-danger'; } else { $clasT = 'text-primary'; };
                    echo '<div class="row">
                            <div class="col-8 border bg-light '.$clasT.'"><small>'.$registro['Material'].' '.$registro['Denominacion'].'</small></div>
                            <div class="col-2 border bg-light '.$clasT.'"><small>'.$registro['Cajas'].'</small></div>
                            <div class="col-2 border bg-light '.$clasT.'"><small>'.$registro['Unidades'].'</small></div>
                         </div>';
		...
		switch ($FMfcgxwLtkdRLdXFNVNgdJkHCHKrmRpl):
			...
			case "DepositosBancarios":
				...
				break;
			case "truck_load":
				if (isset($_GET['ruta'],$_GET['fecha'],$_GET['viaje'])){
				//	truck__load($_GET['ruta'],$_GET['fecha'],$_GET['viaje']);
					}
				break;				
			default:
